...
//TODO
// Output all info we have, and skip "settings"
// Apply what was learned in tuner (apply FFT style multiplying the strength at following harmonics together)
// Perform actual FFT on a good window size - would repeating data here help?
// https://www.bitweenie.com/listings/fft-zero-padding/ for zero pad thoughts
// Investigate zero pad to see what can be done in terms of resolution
// Zero pad site v good. What if we take a sample, and repeat it? Could cause boundary effects by discontinuity right? But what if we do [sample][reverse sample][sample][...]?
// Could this have same effect as windowing to deal with discontinuity or even better? try it

// Consider alternative method: zero pad (or repeat, windowed or otherwise) FFT with various lengths, compare spectra, find the "cleanest" one

// Create [harmonicmap~] or even freqmap~ object. Uses fft to map one freq to another. Needs fft window/current pitch as input.
// Create [scalemap] for midi - easy dynamic loading of scales, map notes to other notes. Will be used for whole guitar and individual strings. Consider at what stage we should introduce mapping as not to compilcate everything. We can view 12TET as not midi but just input.

// High level: Chord detection (triggers)
// High level: (Check axon) create triggers on points on the guitar
// We will use it to detect chord -> map to a just intonated mode for entire guitar
// We will have at least root scale and chord scale as concepts (Root scale for whole guitar, chord scale makes the modal scale to fit non-root notes even further)
// The modal scale thing should simply be a preset for capabilities already existing
// Scale mapper must have modes, smooth-linear (just stretches or contracts notes), smooth-step (essentially autotune but still allows vibrato), step (autotune no vibrato)

//Support scala .scl files
